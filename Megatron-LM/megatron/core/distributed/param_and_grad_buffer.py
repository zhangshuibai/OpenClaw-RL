...
# PyTorch < 2.2 NCCL backend does not support coalesced collectives
# (reduce_scatter_tensor_coalesced / all_gather_into_tensor_coalesced).
# Detect at import time and fall back to nullcontext (sequential ops).
_COALESCING_MANAGER_AVAILABLE = False
try:
    from torch.distributed import _coalescing_manager as _torch_coalescing_manager

    # The function exists since PyTorch 2.0, but the NCCL backend only
    # gained actual support for coalesced reduce-scatter in PyTorch 2.2+.
    # Probe by checking for the C++ symbol on ProcessGroupNCCL.
    _pg_nccl = getattr(
        torch.distributed.distributed_c10d, "ProcessGroupNCCL", None
    )
    if _pg_nccl is not None and hasattr(_pg_nccl, "reduce_scatter_tensor_coalesced"):
        _COALESCING_MANAGER_AVAILABLE = True
except ImportError:
    pass


def _coalescing_manager(group, async_ops=False):
    if _COALESCING_MANAGER_AVAILABLE:
        return _torch_coalescing_manager(group, async_ops=async_ops)
    return nullcontext()
