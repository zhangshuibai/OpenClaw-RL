...
# SWE-Bench RL training (Qwen3-8B) on 2 nodes with REMOTE Docker exec.
...
mkdir -p "${LOG_DIR}"
RUN_LOG=${RUN_LOG:-"${LOG_DIR}/run_swe_rl_8b_remote_2nodes_${RUN_TIMESTAMP}.log"}
...
if [[ ${MLP_ROLE_INDEX} -eq 0 ]]; then
  SWE_POOL_LOG=${SWE_POOL_LOG:-"${LOG_DIR}/swe_env_pool_server_2nodes.log"}
...
CKPT_ARGS=(
  ...
  --ref-load "${REF_LOAD}"
  --save "${SAVE_CKPT:-${EXPORT_ROOT}/ckpt/swe-rl-8b-remote-2nodes_${RUN_TIMESTAMP}}"
  --save-interval 5
...
if [ -n "${WANDB_KEY_VALUE}" ]; then
  WANDB_ARGS=(
    ...
    --wandb-group qwen3-8B-rl_swe_remote_2nodes
...
SWE_SAVE_TRAJ_DIR=${SWE_SAVE_TRAJ_DIR:-${EXPORT_ROOT}/swe_rollouts/swe-rl-8b-remote-2nodes_${RUN_TIMESTAMP}}
...
RAY_JOB_SUBMISSION_ID=${RAY_JOB_SUBMISSION_ID:-"swe_rl_8b_remote_2nodes_$(date +%Y%m%d_%H%M%S)"}
